...
async def _(event):
    ...
    #packname = f"CAGEX PACK"
    #packshortname = f"CAGEX_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1263617196:
        packname = f"@xzendercage Packs 🎭"
        packshortname = "xzendercage"
    else:
        packname = f"{user.username}'s {pack}"
        packshortname = f"CAGEX_{userid}_Pack"
    await moods.edit("`Wew ! I Love That Sticker ! Mind If i Kang It ?`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@CAGEXOT.png"
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            packname = f"xzendercage Ka Pack"
            packshortname = "xzendercageisgreat"
        else:
            packname = f"{user.username}'s {pack}"
            packshortname = f"CAGEX_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 948408212:
                       # packname = f"{user.username}'s {pack}"
                       # packshortname = "Vol._{pack}_CAGEX_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_CAGEX{userid}"
                        #packshortname = f"Vol._{pack}_CAGEX_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                        await moods.edit("**Pack No. **" + str(prevv) + "** full! Making a new Pack, Vol. **" + str(pack))
                        if is_a_s:
                            response = await silently_send_message(bot_conv, "/newanimated")
                        else:
                            response = await silently_send_message(bot_conv, "/newpack")
                        ...
                        await moods.edit("Pack No. " + str(prevv) + " full! Switching to Vol. " + str(pack))
                        await silently_send_message(bot_conv, "/addsticker")
                        await silently_send_message(bot_conv, packshortname)                                                                            
                        await bot_conv.send_file(
                            file=uploaded_sticker,
                ...
                await silently_send_message(bot_conv, sticker_emoji)
                await silently_send_message(bot_conv, "/done")
    await moods.edit(f"**Sticker Has Been Kanged SucessFully And Can Be Found** [Here](t.me/addstickers/{packshortname})")
...
async def _(event):
    ...
    await moods.edit(f"**Sticker Title:** `{get_stickerset.set.title}\n`"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await moods.edit(f"Downloading {sticker_set.set.count} sticker(s) to .{Config.TMP_DOWNLOAD_DIRECTORY}{sticker_set.set.short_name}...")
