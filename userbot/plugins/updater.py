...
UPSTREAM_REPO_URL = "https://github.com/xzendercage/cageuserx"
    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await event.edit(
                ...
                "Visit @xzendercage For More Support `"
            )
            repo.__del__()
            return
...
async def upstream(event):
    ...
    if conf == "" and force_update is False:
        ...
        await event.delete()
        return await event.respond('Update USERX Userbot By `.update deploy` // `.update now`')
