...
async def download(target_file):
    ...
    USERX = await edit_or_reply(target_file ,"`Processing ...`")
    await USERX.edit("Processing using userbot server ( ◜‿◝ )♡")
    ...
    if "|" in input_str:
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff %
                    ...
                    await USERX.edit(current_message)
                    display_message = current_message
            except Exception as e:
                LOGS.info(str(e))
        if downloader.isSuccessful():
            await USERX.edit("Downloaded to `{}` successfully !!".format(
                downloaded_file_name))
        else:
            await USERX.edit("Incorrect URL\n{}".format(url))
        ...
        try:
            ...
            await USERX.edit(str(e))
        else:
            await USERX.edit("Downloaded to `{}` successfully !!".format(
                downloaded_file_name))
    else:
        await USERX.edit("Reply to a message to download to my local server.")
